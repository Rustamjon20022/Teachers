...
export default () => {
	...
	const handleNavigate = (id) => {
		navigate("/bosqich/editbosqich", { state: id });
	};

	const navigateAdd = () => {
		navigate("/bosqich/addbosqich");
	};

	const handleDelete = (id) => {
		axios.delete(`http://localhost:8080/api/steps/${id}`).then(() => {
			getdata();
		});
	};
	// console.log(data);
		...
		<>
			...
			<Card>
				<Table aria-label="collapsible table">
					<TableHead>
						<TableRow>
							{["#", "BOSQICH", "MALUMOT", "BOSHQARUV"].map((item, id) => (
								<TableCell
									key={id}
									align="left"
